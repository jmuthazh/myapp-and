...
public class ResultFragment extends Fragment {
	...
	private LinearLayout mresultFragment;
	...
	public void onViewCreated(View v, Bundle savedInstanceState) {
		...
		if (mFaceModel == null)
			...
			double matchScore = Double.parseDouble(mFaceModel.getMatchScoreString());
			double selfScore = (1-(1/Math.pow(10,matchScore)));
			String formatValue = String.format("%.2f", selfScore);
			mFaceMatchScoreLabel.setText(formatValue);
		}
	...
	/*	mDateIssuedLabel = (TextView)v.findViewById(R.id.issued);
		...
		mresultFragment = (LinearLayout)v.findViewById(R.id.result_fragment);
		mFailureLabel = (TextView)v.findViewById(R.id.failure);
		if(mIdModel.didPass()){
			/*mStatus = (TextView)v.findViewById(R.id.status);
			mStatus.setText("PASS");*/
			mresultFragment.setBackgroundResource(R.color.fed_green);
		}else if(mIdModel.didFail()){
			/*mStatus = (TextView)v.findViewById(R.id.status);
			mStatus.setText("FAIL");*/
			mresultFragment.setBackgroundResource(R.color.error);
		}else if(mIdModel.isPending()){
		/*	mStatus = (TextView)v.findViewById(R.id.status);
			mStatus.setText("PENDING");*/
			mresultFragment.setBackgroundResource(R.color.pending);
		}else if(mIdModel.isUnknown()){
			/*mStatus = (TextView)v.findViewById(R.id.status);
			mStatus.setText("UNKNOWN");*/
			mresultFragment.setBackgroundResource(R.color.unknown);
		}
